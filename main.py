    ...
    for x in filters:
        if message.channel.id == x[0]:
            if message.attachments or re.search(urlRegex, message.content):
                ...
                c.execute(sql,val)
                conn.commit()
                await doLog(f"Thread auto-generated by {message.author.name}", client)
            elif message.type.pins_add:
                pass
            elif message.author.guild_permissions.manage_channels:
                await doLog(f"Message passed ({message.author.name})", client)
            else:
                try:
                ...
                await doLog(f"Deleted message from {message.author.name}", client)
